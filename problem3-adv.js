...
const filterListOfUsersWithMovies = ({
  users,
  movies,
  startDate = "1888-01-01 00:00:00", //First movie release
  endDate = todayString,
  rate = 0,
}) => {
  const usersWithMovies = users.map((user) => {
    const userMovies = movies.filter(
      (movie) =>
        ...
        Date.parse(movie.watched) >= Date.parse(startDate) &&
        Date.parse(movie.watched) <= Date.parse(endDate) &&
        movie.rate >= rate
    );

    const averageRate =
      userMovies.reduce((accumulator, movie) => accumulator + movie.rate, 0) /
      userMovies.length;
    ...
    return {
      ...
      rate: isNaN(averageRate) ? 0 : averageRate.toFixed(2),
    };
  });
